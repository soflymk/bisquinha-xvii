...
io.on('connection', (socket: any) => {
  ...
  const doQueueSwap = (roomId: string, losingTeam: 1 | 2) => {
    ...
    if (specs.length === 1) {
      // Espectador escolhe quem da dupla perdedora sai
      setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      for (let i = 0; i < losingSlots.length; i++) {
        ...
        // Perdedor vai pro final da fila
  ...
  socket.on('initiate_kick', ({ roomId, targetId }: { roomId: string; targetId: string }) => {
    const game = activeGames[roomId];
    if (!game) return;
    if (!game.slots.includes(userId)) return; // só jogadores da mesa
  ...
  socket.on('cast_kick_vote', ({ roomId, approve }: { roomId: string; approve: boolean }) => {
    ...
    if (vote.votes.includes(userId)) return; // já votou
  ...
  socket.on('play_card', ({ roomId, cardId }: { roomId: string, cardId: string }) => {
    ...
    // Regra: Ás do trunfo não pode ser jogado antes do 7 do trunfo (exceto endgame)
    ...
    // Marcar que o 7 do trunfo foi jogado
  ...
  socket.on('disconnect', () => {
    ...
    for (const roomId in activeGames) {
      const game = activeGames[roomId];
      // Cancelar votação se o iniciador desconectou
      ...
      // Remover espectador desconectado
      const specIdx = game.spectators.findIndex(s => s.userId === userId);
      if (specIdx !== -1) {
        const nick = game.spectators[specIdx].nickname;
        game.spectators.splice(specIdx, 1);
